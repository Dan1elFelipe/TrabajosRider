...
function agregarDatos() {
    return new Promise((si, no) => {
        ...
        if (!identificacion || !nombre || isNaN(nota1) || isNaN(nota2) || isNaN(nota3)) {
            no("Por favor, complete todos los campos correctamente.");
        } else {
            const id = Date.now();
...
function eliminarDatosAsync(id) {
    return new Promise((si, no) => {
...
function guardar(id) {
    guardarDatosAsync(id);
}

function eliminar(id) {
    eliminarDatosAsync(id);
}